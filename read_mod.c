	...
	int p_num_samples)
{
	...
	if (retcode == SUCCESS) {
		for (i=0; (i<MAX_CHANNELS && (retcode == SUCCESS)); i++) {
			if (p_module->default_channel_stereo[i] < 1 || p_module->default_channel_stereo[i] > 8) {
				fprintf(stderr,"Modfile corrupt - default stereo position %d invalid\n",
					p_module->default_channel_stereo[i]);
				retcode = FILE_CORRUPT;
			}
		}
	}
	...
	if (retcode == SUCCESS) {
		for (i=0; (i<NUM_PATTERNS && (retcode == SUCCESS)); i++) {
			if (p_module->pattern_length[i] < 1 || p_module->pattern_length[i] > MAX_PATTERNLENGTH) {
				fprintf(stderr,"Modfile corrupt - pattern length %d invalid\n", p_module->pattern_length[i]);
				retcode = FILE_CORRUPT;
			}
		}
	}
...
#ifdef DEVELOPING
				if (retcode == SUCCESS && (p_sample->repeat_offset + p_sample->repeat_length) > p_sample->sample_length) {
					printf(
						"Warning - sample %d repeat offset + repeat length (%d) greater than sample length %d\n",
						i, (p_sample->repeat_offset + p_sample->repeat_length), p_sample->sample_length);
				}
#endif
			}
		}
