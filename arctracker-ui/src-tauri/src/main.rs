...
const INSERT_SEQUENCE_BEFORE_WITH_NEW_REQUESTED_EVENT: &str = "insert-sequence-before-with-new-requested";
const INSERT_SEQUENCE_AFTER_WITH_NEW_REQUESTED_EVENT: &str = "insert-sequence-after-with-new-requested";
...
fn setup_app<R: Runtime>(app: &mut App<R>) -> Result<(), Box<dyn Error>> {
    ...
    app.on_menu_event(|app_handle, event| {
        match event.id().as_ref() {
            ...
            INSERT_SEQUENCE_BEFORE_MENU_ID => request_event(app_handle, INSERT_SEQUENCE_BEFORE_REQUESTED_EVENT),
            INSERT_SEQUENCE_AFTER_MENU_ID => request_event(app_handle, INSERT_SEQUENCE_AFTER_REQUESTED_EVENT),
            INSERT_SEQUENCE_BEFORE_WITH_NEW_MENU_ID => request_event(app_handle, INSERT_SEQUENCE_BEFORE_WITH_NEW_REQUESTED_EVENT),
            INSERT_SEQUENCE_AFTER_WITH_NEW_MENU_ID => request_event(app_handle, INSERT_SEQUENCE_AFTER_WITH_NEW_REQUESTED_EVENT),
            DELETE_SEQUENCE_POSITION_MENU_ID => request_event(app_handle, DELETE_SEQUENCE_POSITION_REQUESTED_EVENT),
...
fn install_menu<R: Runtime>(app: &App<R>) -> tauri::Result<()> {
    ...
    let menu = Menu::with_items(app, &[&app_menu, &file_menu, &edit_menu, &module_menu, &sequence_menu, &pattern_menu])?;
    app.set_menu(menu)?;
    Ok(())
}
...
fn build_app_menu<R: Runtime>(
    ...
    let quit = MenuItem::with_id(app, QUIT_MENU_ID, "Quit Arctracker", true, Some("CmdOrCtrl+Q"))?;
    let about = PredefinedMenuItem::about(app, Some("About Arctracker"), None)?;
    let separator = PredefinedMenuItem::separator(app)?;
    Submenu::with_items(app, "Arctracker", true, &[&about, &separator, &quit])
}

fn build_file_menu<R: Runtime>(
    app: &App<R>,
) -> tauri::Result<Submenu<R>> {
    let new_module = MenuItem::with_id(app, NEW_MODULE_MENU_ID, "New Module", true, Some("CmdOrCtrl+N"))?;
    let open_module = MenuItem::with_id(app, OPEN_MODULE_MENU_ID, "Open Module...", true, Some("CmdOrCtrl+O"))?;
    let save_module = MenuItem::with_id(app, SAVE_MODULE_MENU_ID, "Save Module", true, Some("CmdOrCtrl+S"))?;
    let save_module_as = MenuItem::with_id(app, SAVE_MODULE_AS_MENU_ID, "Save Module As...", true, Some("Shift+CmdOrCtrl+S"))?;
    let export_module_audio = MenuItem::with_id(app, EXPORT_AUDIO_MENU_ID, "Export Audio...", true, Some("CmdOrCtrl+B"))?;
    let separator = PredefinedMenuItem::separator(app)?;
    Submenu::with_items(app, "File", true, &[&new_module, &open_module, &save_module, &save_module_as, &separator, &export_module_audio])
}
...
fn build_edit_menu<R: Runtime>(
    ...
    let cut_events = MenuItem::with_id(app, CUT_EVENTS_MENU_ID, "Cut Events", true, Some("CmdOrCtrl+X"))?;
    let copy_events = MenuItem::with_id(app, COPY_EVENTS_MENU_ID, "Copy Events", true, Some("CmdOrCtrl+C"))?;
    let paste_events = MenuItem::with_id(app, PASTE_EVENTS_MENU_ID, "Paste Events", true, Some("CmdOrCtrl+V"))?;
    let cut_track = MenuItem::with_id(app, CUT_TRACK_MENU_ID, "Cut Track", true, Some("Shift+F3"))?;
    let copy_track = MenuItem::with_id(app, COPY_TRACK_MENU_ID, "Copy Track", true, Some("Shift+F4"))?;
    let paste_track = MenuItem::with_id(app, PASTE_TRACK_MENU_ID, "Paste Track", true, Some("Shift+F5"))?;
    let cut_pattern = MenuItem::with_id(app, CUT_PATTERN_MENU_ID, "Cut Pattern", true, Some("CmdOrCtrl+F3"))?;
    let copy_pattern = MenuItem::with_id(app, COPY_PATTERN_MENU_ID, "Copy Pattern", true, Some("CmdOrCtrl+F4"))?;
    let paste_pattern = MenuItem::with_id(app, PASTE_PATTERN_MENU_ID, "Paste Pattern", true, Some("CmdOrCtrl+F5"))?;
    let separator = PredefinedMenuItem::separator(app)?;
    Submenu::with_items(app, "Edit", true, &[&undo, &redo, &separator, &cut_events, &copy_events, &paste_events, &separator, &cut_track, &copy_track, &paste_track, &separator, &cut_pattern, &copy_pattern, &paste_pattern])
}

fn build_module_menu<R: Runtime>(
    app: &App<R>,
) -> tauri::Result<Submenu<R>> {
    let edit_details = MenuItem::with_id(app, EDIT_DETAILS_MENU_ID, "Edit Details", true, None::<String>)?;
    let set_track_count = MenuItem::with_id(app, SET_TRACK_COUNT_MENU_ID, "Set Track Count", true, Some("CmdOrCtrl+T"))?;
    let set_tempo = MenuItem::with_id(app, SET_TEMPO_MENU_ID, "Set Tempo", true, None::<String>)?;
    Submenu::with_items(app, "Module", true, &[&edit_details, &set_track_count, &set_tempo])
}

fn build_sequence_menu<R: Runtime>(
    app: &App<R>,
) -> tauri::Result<Submenu<R>> {
    let insert_before = MenuItem::with_id(app, INSERT_SEQUENCE_BEFORE_MENU_ID, "Insert Position Before", true, Some("F1"))?;
    let insert_after = MenuItem::with_id(app, INSERT_SEQUENCE_AFTER_MENU_ID, "Insert Position After", true, Some("F2"))?;
    let insert_before_with_new = MenuItem::with_id(app, INSERT_SEQUENCE_BEFORE_WITH_NEW_MENU_ID, "Insert Before With New Pattern", true, Some("Shift+F1"))?;
    let insert_after_with_new = MenuItem::with_id(app, INSERT_SEQUENCE_AFTER_WITH_NEW_MENU_ID, "Insert After With New Pattern", true, Some("Shift+F2"))?;
    let delete_position = MenuItem::with_id(app, DELETE_SEQUENCE_POSITION_MENU_ID, "Delete Position", true, None::<String>)?;
    Submenu::with_items(app, "Sequence", true, &[&insert_before, &insert_after, &insert_before_with_new, &insert_after_with_new, &delete_position])
}

fn build_pattern_menu<R: Runtime>(
    app: &App<R>,
) -> tauri::Result<Submenu<R>> {
    let set_length = MenuItem::with_id(app, SET_PATTERN_LENGTH_MENU_ID, "Set Length", true, Some("CmdOrCtrl+L"))?;
